...
func makeVillagerEnv(t *testing.T) (*State, *Env) {
	...
	w.AddStructure(lsOrigin.X, lsOrigin.Y, 4, 4, LogStorage, testLogStorageDef{})

	// House at (20, 20) — 2×2
	hOrigin := Point{X: 20, Y: 20}
	w.AddStructure(hOrigin.X, hOrigin.Y, 2, 2, House, testHouseDef{})
...
func TestVillagerFetchesAndDelivers(t *testing.T) {
	...
	s.World.AddStructure(fhOrigin.X, fhOrigin.Y, 2, 2, FoundationHouse, testHouseDef{})
...
func TestNearestClearTileAdjacent(t *testing.T) {
	w := NewWorld(20, 20)
	w.AddStructure(5, 5, 4, 4, LogStorage, testLogStorageDef{})
...
func TestCountStructureInstances(t *testing.T) {
	...
	w.AddStructure(2, 2, 2, 2, House, testHouseDef{})
