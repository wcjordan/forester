...
| Animation | Rows  | Frames | Trigger |
|-----------|-------|--------|---------|
| Walk      | 8–11  | 8 (0–7 cycling) | WASD / arrow key held |
| Slash     | 12–15 | 6      | `Player.LastHarvestAt` within 500ms |
| Thrust    | 4–7   | 8      | `Player.LastBuildAt` within 500ms |

Priority: Slash > Thrust > Walk > Idle (frame 0 of current facing row).

### Code changes

- Add `LastHarvestAt time.Time` and `LastBuildAt time.Time` to `Player` in `game/player.go` (small game-layer change; set in `game/resources/wood.go` and `game/structures/house.go`).
- Add `playerMoving bool`, `animTick int` to `EbitenGame` in `render/ebiten_model.go`.
- Add `dirFrom(dx, dy int) int` and `spriteForPlayer(dir, animRow, frame int) drawArgs` in `render/sprites.go`.
...
- Villager task state (`VillagerTask`) can optionally drive slash (chopping) and thrust (delivering) animations using the same 500ms window approach.
