...
//  6. Navigate east×4 then south×5 to (52,50).
...
//   - Phase 7: the house foundation spawns at (52,50) — the player's position when
//     Harvest fires on the first tick. Player is "inside" the 2×2 foundation; the
//     adjacent tile (53,50) drives the per-tick deposits.
...
func TestHouseWorkflow(t *testing.T) {
	...
	// ── Phase 6: Navigate to (52,50) ─────────────────────────────────────────
	// Route: east×4 through mixed terrain (some Forest at x=48,49,51,52),
	// then south×5 through Grassland to (52,50).
	...
	announcePhase(m, "Phase 6: Navigate to (52,50)")
	for range 4 {
	...
	if g.State.Player.X != 52 || g.State.Player.Y != 50 {
		t.Fatalf("phase 6: expected player at (52,50), got (%d,%d)",
			g.State.Player.X, g.State.Player.Y)
	}

	// ── Phase 7: Build house ──────────────────────────────────────────────────
	// First tick fires Harvest → maybeSpawnFoundation sees stores≥50 → spawns
	// house foundation at (52,50) (the player's position — isValidArea passes
	// because no structure is there yet). Within the same tick, TickAdjacentStructures
	// sees (53,50) adjacent to player, fires OnPlayerInteraction, deposits 1 wood.
	...
	// House tile at (52,50) (where foundation spawned) must show House structure.
	houseTile := g.State.World.TileAt(52, 50)
	if houseTile == nil || houseTile.Structure != game.House {
		t.Errorf("phase 8: expected House at (52,50), got %v", houseTile)
	}
